# Living Lab

Siemens digital Enlighted IoT sensors

decentralised energy generation and
storage, renewable energy and concepts that help change users’ behaviours transform
the University’s Edgbaston [link](https://www.birmingham.ac.uk/university/building/smart-campus/index.aspx)

#### Functions

Applications may possibly have futuristic and blue skies characters.

Applications may capture, monitor, analyse, visualise, simulate data.

Applications may use gamification, machine learning, data analysis and
dynamic optimisation techniques to change users’ behaviours on campus

Better optimise operations on campus to reach Governmental NetZero emissions targets; and to provide opportunities for raising public awareness etc.

![image-20211122161748278](C:\Users\calvchen\AppData\Roaming\Typora\typora-user-images\image-20211122161748278.png)

More References that relate to Challenge 1 brief description:
Ref1.https://www.birmingham.ac.uk/university/building/smart-campus/index.aspx
Ref2.https://www.birmingham.ac.uk/documents/university/smart-campus/thesmart-
campus.pdf

### Requirements

#### A.

A1. Describe proposed system in 800 words, stating your own
assumptions on the scope of the system. Be precise, concise, and creative!. (**reasonable and interesting**)

A2. State functional and non-funtional requirements of the system.Use
sensible phrasing, grouping and prioritisation as discussed in the sessions.

#### B. UML

Use UML to document the analysis and design of your system to a professional level.

![image-20211122162250394](C:\Users\calvchen\AppData\Roaming\Typora\typora-user-images\image-20211122162250394.png)

![image-20211122162300747](C:\Users\calvchen\AppData\Roaming\Typora\typora-user-images\image-20211122162300747.png)

![image-20211122162307619](C:\Users\calvchen\AppData\Roaming\Typora\typora-user-images\image-20211122162307619.png)

#### C. Software Architecture Style, Modelling and Evaluation

![image-20211122162332132](C:\Users\calvchen\AppData\Roaming\Typora\typora-user-images\image-20211122162332132.png)

#### D. Software Testing

![image-20211122162353520](C:\Users\calvchen\AppData\Roaming\Typora\typora-user-images\image-20211122162353520.png)

#### E. Usability and Prototyping (Unit 6)

![image-20211122162415686](C:\Users\calvchen\AppData\Roaming\Typora\typora-user-images\image-20211122162415686.png)

![image-20211122162420355](C:\Users\calvchen\AppData\Roaming\Typora\typora-user-images\image-20211122162420355.png)

#### F. Ethics and Professional Practice – (Unit 8) – self-reading:

Your attempt should follow ethical and responsible design. Provide appraisal using the IEEE/ACM software engineering code of ethics. Your appraisal should not exceed 500 words. (2 points)



#### Scope of the System



# 3D Map

#### Functional

**REQ1.** User accounts



REQ2. 3-Dimension Map with High degree of freedom

The system is much of based on the 3D map that can rotate 360 degree horizontally with different kinds of view:

View: The four views: A) overhead, B) first-person, C) third- person-high, D) third-person-low [link](https://www.researchgate.net/figure/The-four-views-A-overhead-B-first-person-C-third-person-high-D-third-person-low_fig1_221474898)

<img src="C:\Users\calvchen\AppData\Roaming\Typora\typora-user-images\image-20211122164519781.png" alt="image-20211122164519781" style="zoom:25%;" />





REQ. Real-time Data and Visualization



 

REQ. Energy Management & Optimization System



REQ. Personal Recommend System

The system will ask some simple questions when the account first log-in, such as your perferred diet time and sport time and your dietary preference.

Some data will also be collected and only will be used to improve the user experience that can recommend you where to eat during a very tight scheduling noon or can remind you to book study room cuz you have a meeting this afternoon.







REQ. Notification System

The system will base on the optimization algorithms to give some advice for reducing the total power consumption of the UoB. 











REQ. Concurrency

![image-20211122171531424](C:\Users\calvchen\AppData\Roaming\Typora\typora-user-images\image-20211122171531424.png)



REQ. 







#### Non-functional







# Energy Saving

[link](https://www.birmingham.ac.uk/university/building/smart-campus/journey-to-net-zero-carbon.aspx)

Anonymous data and information gathered from these new technologies will help the University to identify potential building issues before they  impact end-user experience. By spotting possible issues before they  become a major problem, this will help to offset some of the reactive  pressures on various University teams, enabling staff to better plan,  prepare and make informed decisions on how best to take corrective  action.



#### Functional

